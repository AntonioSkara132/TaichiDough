...
import hashlib
isolation = {}
for name in ('solver', 'state', 'spectral'):
    module = importlib.import_module('experiments.differentiable_mpm.' + name)
    path = Path(module.__file__).resolve()
    assert path.is_relative_to(Path(__file__).resolve().parent / 'snapshot')
    isolation[name] = {'path': str(path), 'sha256': hashlib.sha256(path.read_bytes()).hexdigest()}
print('ISOLATED NUMERICAL IMPORTS: ' + json.dumps(isolation), flush=True)
with (Path(__file__).resolve().parent / 'import_identity.json').open('x') as stream:
    json.dump(isolation, stream, indent=2)
import numpy as np

from experiments.differentiable_mpm.config import EXPERIMENT_ROOT, file_sha256, load_config
...
def main():
    ...
    with reference_policy('frozen'):
        ...
        manifest = {'schema': 'taichidough/table-floor-forward/v1', 'requested_backend': args.backend,
                    ...
                    'limitations': ['User-requested forward parameters; no fitting.',
                                   'Exact registered tools from verified immutable bundle.',
