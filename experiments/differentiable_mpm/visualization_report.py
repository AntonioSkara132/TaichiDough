#!/usr/bin/env python3
"""Standalone experimental depth report, adapted from the production visualizer.

Original scripts/visualize_dynamic_topview_benchmark.py SHA256:
e29f60e92b9fe8a161e3dc05f11ae0962f5ec13d3301017ef7877ae279002fa9
This copy requires no production scripts or simulation runtime.
"""
...
def normalize_report_context(report: dict) -> None:
    ...
    replay["tool_geometry_description"] = (
        "voxelized STL collision surfaces posed in tool-link frames" if collision_mode == "sdf" else
...
def create_report(metrics_path: Path, output_dir: Path, *, run_context=None) -> Path:
    ...
    if run_context is not None:
        report["calibration_run"] = run_context
        selected_frames = set(run_context.get("selection_frames", []))
        for row in report["frames"]:
            row["scoring_role"] = (
                "initialization" if row["source_frame"] == 0 else
                "material fitting" if row["source_frame"] in selected_frames else
                "outside material-selection frames" if selected_frames else
                "selection membership unavailable"
            )
...
<p id="provenance"></p><p id="runcontext"></p><div class="cards" id="cards"></div>
...
if(report.calibration_run){const r=report.calibration_run;$('runcontext').textContent=`Saved run: ${r.run_name}. Physics: ${r.physics_version??'unrecorded'}. Parameters: ${JSON.stringify(r.parameters??{})}. Material-selection labels do not establish independence of geometry preprocessing. This report reads saved evaluation arrays; it does not rerun fitting or simulation.`;}
...
const collisionMode=report.replay.tool_collision||'box',geometryStatus=collisionMode==='sdf'?'tool poses target voxelized STL mesh/link frames':(report.replay.tool_geometry_proxy?'tool collider geometry is a proxy':'tool collider geometry is recorded per tool');
...
$('toolGeometry').textContent=collisionMode==='box'?`Tool names identify separate paths. The box boundary is the unit simulation domain, not a measured table boundary. Solid orange wireframes show ${report.replay.tool_geometry_description}; dashed orange wireframes add the configured contact padding. Neither denotes a confirmed contact event.`:collisionMode==='sdf'?`Tool names identify separate paths. Poses target ${report.replay.tool_geometry_description}; this report does not contain mesh vertices, so it deliberately renders no proxy-box outline or inferred contact event.`:'Tool names identify separate paths. Tool collision is disabled; no contact geometry or inferred contact event is rendered.';
...
function select(index){selected=Math.max(0,Math.min(rows.length-1,index));$('timeline').value=selected;const r=rows[selected];for(const name of['real','sim','overlay','residual'])$(name).src=r.images[name];$('readout').textContent=`Retained frame ${r.source_frame} (original ${r.original_source_frame}) | source ${fmt(r.source_timestamp_s,6)} s | elapsed ${fmt(r.time_s,6)} s | simulation ${fmt(r.sim_time_s,6)} s | lag ${fmt(r.pairing_error_s==null?null:r.pairing_error_s*1000,3)} ms | substeps ${r.simulation_step??'N/A'} | ${r.status} | ${r.phase} | ${r.scoring_role??'selection membership unavailable'}`;const m=r.metrics;$('support').textContent=m?`Common ${m.common_visible_pixels}; real only ${m.real_only_pixels}; simulation only ${m.sim_only_pixels}. Both absent or tool-occluded pixels are not distinguishable with the available dough-only capture.`:'No paired metric support. Missing data is not zero error.';loadPixelData();drawVisibility();drawTools();for(const obj of chartObjects){obj.index=selected;drawChart(obj)}}
