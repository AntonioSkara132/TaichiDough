...
def run(args):
    ...
    paths += mesh_paths
    paths += [REPOSITORY_ROOT / 'meshes/tool_collision_meshes_v1.json',
              EXPERIMENT_ROOT / 'data/episode18_table_aligned_v1/episode18_table_aligned_coulomb.json',
              EXPERIMENT_ROOT / 'reference/taichi_viscoelastic_mpm_scene.py']
    hashes = {str(path): file_sha256(path) for path in paths}
    if hashes[str(mesh_paths[2])] != '65e2afa80894c45dde9ef1791786c8652d1de22a0a6b39de098fb2567552b251':
        raise ValueError('Authorized collision mesh changed; inspect before fitting')
    raw_mesh = mesh_paths[2].read_bytes()
    records = np.frombuffer(raw_mesh, dtype=[('normal', '<f4', (3,)),
        ('vertices', '<f4', (3, 3)), ('attribute', '<u2')], offset=84)
    vertices = records['vertices'].astype(float)
    crosses = np.cross(vertices[:, 1] - vertices[:, 0], vertices[:, 2] - vertices[:, 0])
    audit = {'target_sha256': hashes[str(mesh_paths[2])], 'triangles': len(vertices),
        'raw_bounds_mm': [vertices.min((0, 1)).tolist(), vertices.max((0, 1)).tolist()],
        'signed_volume_mm3': float(np.sum(vertices[:, 0] * np.cross(vertices[:, 1], vertices[:, 2])) / 6),
        'degenerate_triangles': int(np.sum(np.linalg.norm(crosses, axis=1) < 1e-12)),
        'stored_normals_agree_with_winding_fraction': float(np.mean(np.sum(crosses * records['normal'], axis=1) > 0)),
        'scale_m_per_stl_unit': .001, 'additional_reflection_applied': False,
        'visibility': 'Depth-only sampled z-buffer; no normal-facing rejection. Inward winding does not remove faces. Point-to-plane normal sign reverses both Jacobian and residual, leaving ICP updates unchanged.',
        'coordinate_assumption': 'raw_stl_visual as declared in existing manifest; new bounds match Z reflection of old bounds. Apply existing visual RPY and origin once.'}
    save_json(output / 'mesh_audit.json', audit)
    ...
    for j, (origin, rpy) in enumerate(specifications):
        v, _ = simulator.load_binary_stl(mesh_paths[2 if j == 0 else j], .001)
        tri = simulator.mesh_in_tool_frame(v, origin, simulator.rpy_to_matrix(rpy)).reshape(-1, 3, 3).astype(float)
        if j == 0:
            save_json(output / 'mesh_transform.json', {'visual_origin_m': origin.tolist(),
                'visual_rpy_rad': rpy.tolist(), 'visual_rotation': simulator.rpy_to_matrix(rpy).tolist(),
                'tool_frame_bounds_m': [tri.min((0, 1)).tolist(), tri.max((0, 1)).tolist()],
                'composition': 'camera_from_source @ source_from_marker(t) @ candidate_marker_from_mesh @ visual_origin_RPY @ scale'})
    ...
    # Results contain a candidate matrix only; no simulation-ready geometry is written.
    all_supported = False
    ...
    report = {'schema': 'taichidough/tool-cloud-registration/v1', 'status': 'completed',
              ...
              'results': results,
              'fitting_target': str(mesh_paths[2]), 'gen3_fitted': False,
              'association_recomputed': True,
              'comparison_note': 'Earlier run fitted the visual STL. This run fits the updated collision STL and recomputes associations; historical metrics are not a controlled geometry-only comparison.',
              'downstream_warning': 'Production collision manifest and experiment expected hashes predate the authorized edit. No manifest verification is bypassed for simulation; no simulation-ready config is written.',
